var tensor_tool = {
    ...
    quat: class {
        ...
        rev() {
            return new this.constructor(-this.s, this.i, this.j, this.k);
        }
        ...
        ipo(q2, r = 0.5) {
            ...
            return new this.constructor(s * div, i * div, j * div, k * div);
        }
    ...
    frame: class {
        ...
        from_3pt(po, px, py) {
            this.xyz = po;
            this.abc = tensor_tool.quat.gen_xy(px.sub(po), py.sub(po));
        }

        gap(f) {
            return {
                x: (this.xyz.sub(f.xyz)).len(),
                y: tensor_tool.quat.interval(this.abc, f.abc)
            };
        }

        rev() {
            return new this.constructor(
                this.abc.rev().rot_vec(this.xyz.rev()),
                this.abc.rev());
        }

        on(f) {
            return new this.constructor(f.abc.rot_vec(this.xyz).add(f.xyz), f.abc.cook(this.abc));
        }

        at(f) {
            return new constructor(f.abc.rev().rot_vec(xyz - f.xyz), f.abc.rev().cook(this.abc));
        }
        ...
        trans(p) {
            return this.abc.rot_vec(p).add(this.xyz);
        }
        measure(p) {
            return this.abc.rev().rot_vec(p.sub(this.xyz));
        }

        x() { return this.abc.rot_vec(new tensor_tool.vec3(1.0, 0.0, 0.0)); }
        y() { return this.abc.rot_vec(new tensor_tool.vec3(0.0, 1.0, 0.0)); }
        z() { return this.abc.rot_vec(new tensor_tool.vec3(0.0, 0.0, 1.0)); }
...

{
    ...
    cquat = {
        ...
        x: deg => { return new quat(cvec3.x, cfloat.to_arc * deg); },
        y: deg => { return new quat(cvec3.y, cfloat.to_arc * deg); },
        z: deg => { return new quat(cvec3.z, cfloat.to_arc * deg); },
    }
